...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      other_stops.name, b.company, b.num
    FROM
      stops AS craiglockhard_stop
    JOIN
      routes a ON craiglockhard_stop.id = a.stop_id
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops AS other_stops ON other_stops.id = b.stop_id
    WHERE
      craiglockhard_stop.name = 'Craiglockhart'
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      b.num, b.company, transfer_stops.name, c.num, c.company
    FROM
      stops AS craiglockhard_stop
    JOIN
      routes a ON craiglockhard_stop.id = a.stop_id
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops transfer_stops ON b.stop_id = transfer_stops.id
    JOIN
      routes c ON transfer_stops.id = c.stop_id
    JOIN
      routes d ON (d.company = c.company AND d.num = c.num)
    JOIN
      stops AS sighthill_stop ON d.stop_id = sighthill_stop.id
    WHERE
      craiglockhard_stop.name = 'Craiglockhart' AND sighthill_stop.name = 'Sighthill'
  SQL
end
